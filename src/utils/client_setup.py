...
def after_request(response: httpx.Response):
    ...
    logger.info(
        f'<localidades - ibge> {request.method} {request.url} {response.status_code} '
        f'{elapsed} {request.extensions.get("request_id")}'
    )

client = httpx.Client(event_hooks={'request':[before_request],'response':[after_request]},transport=transport)
